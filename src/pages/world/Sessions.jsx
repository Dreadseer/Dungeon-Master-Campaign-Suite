...
export default function Sessions() {
  ...
  useEffect(() => {
    if (!selectedId) { setDetail(null); setPlots([]); setReveals([]); return }
    let cancelled = false
    ;(async () => {
      try {
        const [session, allPlots, sessionReveals] = await Promise.all([
...
// ── Detail pane ───────────────────────────────────────────────────────────────
function SessionDetail({ session, plots, reveals, onSaved, onDelete }) {
